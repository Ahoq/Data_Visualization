...
create_fm_en_et<-function(myfile1){
  
  data_set<-read_excel(myfile1)

  data_set$x_cam_pop<-data_set$Campus_Popn_F-data_set$Clinic_Enroll_F
  data_set$fpop_pct <- round((data_set$Clinic_Enroll_F/data_set$Campus_Popn_F)*100)
  data_set$med_en_pct <- round((data_set$Med_Pt_Ct_F/data_set$Clinic_Enroll_F)*100)
  ...
  list_Sponsor_Level_Options <- c(select.list(option_text1,
    ...
    if (list_Sponsor_Level_Options=="Specific Sponsor"){
      ...
      spec_cam_pop_f<-data_set$Campus_Popn_F[which (data_set$Sponsor_OutreachCharts_Name %in% list_uniq_Sponsor)]
      spec_clinic_en_f<-data_set$Clinic_Enroll_F[which (data_set$Sponsor_OutreachCharts_Name %in% list_uniq_Sponsor)]
      ...
      spec_med_en_pct<-data_set$med_en_pct[which (data_set$Sponsor_OutreachCharts_Name %in% list_uniq_Sponsor)]
      
      new_data_set_1 <- data.frame(spec_sitename,spec_cam_pop_f,spec_clinic_en_f,spec_md_pt_ct_f,spec_x_cam_popn,
                                   spec_fpop_pct,spec_med_en_pct)
      
       new_data_1<-melt(as.data.frame(new_data_set_1), id.vars=c('spec_sitename','spec_cam_pop_f'),
                measure.vars=c('spec_x_cam_popn','spec_clinic_en_f'))
       new_data_2<-melt(as.data.frame( new_data_set_1), id.vars=c('spec_sitename','spec_cam_pop_f'),
                measure.vars=c('spec_x_cam_popn','spec_clinic_en_f','spec_md_pt_ct_f','spec_cam_pop_f'))
       new_data_3<-melt(as.data.frame( new_data_set_1), id.vars=c('spec_sitename','spec_cam_pop_f','spec_med_en_pct'),
                measure.vars=c('spec_md_pt_ct_f'))
       new_data_4<-melt(as.data.frame( new_data_set_1), id.vars=c('spec_sitename','spec_cam_pop_f','spec_fpop_pct'),
                measure.vars=c('spec_clinic_en_f'))
      
       new_data_2$dt_lbl <-  new_data_2$spec_cam_pop_f
      
      k<- as.numeric(length(unique(new_data_set_1$spec_sitename)))
      
      if (k>12){
         new_data_4$dt_lbl2<- paste( new_data_4$value,";","\n", new_data_4$spec_fpop_pct,"%")
         new_data_3$dt_lbl3<- paste( new_data_3$value,";","\n", new_data_3$spec_med_en_pct,"%")
      } else if (k<12){
         new_data_4$dt_lbl2<- paste( new_data_4$value,";", new_data_4$spec_fpop_pct,"%")
         new_data_3$dt_lbl3<- paste( new_data_3$value,";", new_data_3$spec_med_en_pct,"%")
      }else if (k==12){
         new_data_4$dt_lbl2<- paste( new_data_4$value,";", new_data_4$spec_fpop_pct,"%")
         new_data_3$dt_lbl3<- paste( new_data_3$value,";", new_data_3$spec_med_en_pct,"%")
      }
       ...
       new_sub_data <- subset( new_data_2, variable == "spec_cam_pop_f")
      ...
      if (list_identification=="Identified"){
        ggplot(data =  new_data_1, aes(x=reorder(spec_sitename,value), value, fill=variable,label=value))+ geom_col(width=.5) + 
          ...
          geom_text(position=position_stack(vjust=.68),size=3,color="#d9d9d9",alpha=0)+
          scale_fill_manual(values=c("#ccc1d4","#604a7b","#d9d9d9"),
                            labels=c("Female Medical Patients Seen (N;%)   ",
                                     "Female Clinic Enrollees   ","Campus Population")
                            ,breaks=c("spec_md_pt_ct_f","spec_clinic_en_f","spec_x_cam_popn"))+
      ...
      cam_pop_f_all_sponsor <-data_set$Campus_Popn_F[which (data_set$Sponsor_OutreachCharts_Name %in% uniq_spec_Sponsor)]
      clinic_en_f_all_sponsor<-data_set$Clinic_Enroll_F[which (data_set$Sponsor_OutreachCharts_Name %in% uniq_spec_Sponsor)]
      md_pt_ct_f_all_sponsor<-data_set$Med_Pt_Ct_F[which (data_set$Sponsor_OutreachCharts_Name %in% uniq_spec_Sponsor)]
      x_cam_popn_all_sponsor<-data_set$x_cam_pop[which (data_set$Sponsor_OutreachCharts_Name %in% uniq_spec_Sponsor)]
      hs_cam_popn_all_sponsor<-data_set$Enroll_Campus_Pop_9to12[which (data_set$Sponsor_OutreachCharts_Name %in% uniq_spec_Sponsor)]
      fpop_pcnt_all_sponsor<-data_set$fpop_pct[which (data_set$Sponsor_OutreachCharts_Name %in% uniq_spec_Sponsor)]
      md_en_pcnt_all_sponsor<-data_set$med_en_pct[which (data_set$Sponsor_OutreachCharts_Name %in% uniq_spec_Sponsor)]
      
       new_data_set_1<-data.frame(sitename_all_sponsor,cam_pop_f_all_sponsor,clinic_en_f_all_sponsor,md_pt_ct_f_all_sponsor,x_cam_popn_all_sponsor,hs_cam_popn_all_sponsor,fpop_pcnt_all_sponsor,md_en_pcnt_all_sponsor)
      
       new_data_1<-melt(as.data.frame( new_data_set_1), id.vars=c('sitename_all_sponsor','cam_pop_f_all_sponsor','hs_cam_popn_all_sponsor'),
                measure.vars=c('x_cam_popn_all_sponsor','clinic_en_f_all_sponsor'))
       new_data_1<- na.omit( new_data_1)
      
       new_data_2<-melt(as.data.frame( new_data_set_1), id.vars=c('sitename_all_sponsor','cam_pop_f_all_sponsor','hs_cam_popn_all_sponsor'),
                measure.vars=c('x_cam_popn_all_sponsor','clinic_en_f_all_sponsor','md_pt_ct_f_all_sponsor','cam_pop_f_all_sponsor'))
       new_data_2<- na.omit( new_data_2)
      
       new_data_3<-melt(as.data.frame( new_data_set_1), id.vars=c('sitename_all_sponsor','cam_pop_f_all_sponsor','hs_cam_popn_all_sponsor','md_en_pcnt_all_sponsor'),
                measure.vars=c('md_pt_ct_f_all_sponsor'))
       new_data_3<- na.omit( new_data_3)
      
       new_data_4<-melt(as.data.frame( new_data_set_1), id.vars=c('sitename_all_sponsor','cam_pop_f_all_sponsor','hs_cam_popn_all_sponsor','fpop_pcnt_all_sponsor'),
                measure.vars=c('clinic_en_f_all_sponsor'))
      ...
      new_data_set_2<-data.frame(spec_sitename,spec_cam_pop_f,spec_clinic_en_f,spec_md_pt_ct_f,spec_x_cam_popn,spec_hs_cam_popn,spec_fpop_pct,spec_med_en_pct)
      
       new_data_5<-melt(as.data.frame(new_data_set_2), id.vars=c('spec_sitename','spec_cam_pop_f','spec_hs_cam_popn'),
                measure.vars=c('spec_x_cam_popn','spec_clinic_en_f'))
      
      
       new_data_6<-melt(as.data.frame(new_data_set_2), id.vars=c('spec_sitename','spec_cam_pop_f','spec_hs_cam_popn'),
                measure.vars=c('spec_x_cam_popn','spec_clinic_en_f','spec_md_pt_ct_f','spec_cam_pop_f'))
      
      
       new_data_7<-melt(as.data.frame(new_data_set_2), id.vars=c('spec_sitename','spec_cam_pop_f','spec_hs_cam_popn','spec_med_en_pct'),
                measure.vars=c('spec_md_pt_ct_f'))
      
      
       new_data_8<-melt(as.data.frame(new_data_set_2), id.vars=c('spec_sitename','spec_cam_pop_f','spec_hs_cam_popn','spec_fpop_pct'),
                measure.vars=c('spec_clinic_en_f'))
      ...
      new_data_hs_7<-subset( new_data_7, spec_hs_cam_popn !=0)
      
      new_data_hs_8<-subset( new_data_8, spec_hs_cam_popn !=0)
      
      new_data_hs_4$dt_lbl2<- paste(new_data_hs_4$value,";","\n",new_data_hs_4$fpop_pcnt_all_sponsor,"%")
      new_data_hs_3$dt_lbl3<- paste(new_data_hs_3$value,";","\n",new_data_hs_3$md_en_pcnt_all_sponsor,"%")
      new_data_hs_8$dt_lbl4<- paste(new_data_hs_8$value,";","\n",new_data_hs_8$spec_fpop_pct,"%")
      new_data_hs_7$dt_lbl5<- paste(new_data_hs_7$value,";","\n",new_data_hs_7$spec_med_en_pct,"%")
      
      new_data_hs_2$dt_lbl<- new_data_hs_2$cam_pop_f_all_sponsor
      
       new_sub_data <- subset(new_data_hs_2, variable == "cam_pop_f_all_sponsor")
      ...
      if (list_identification=="Identified"){
        ggplot(data = new_data_hs_1, aes(x=reorder(sitename_all_sponsor,value), value, fill=variable,label=value))+ geom_col(width=.8) + 
          ...
          scale_fill_manual(values=c("#b6d9f9","#e5d7ef","#7594af","#604a7b","#848383","#d9d9d9"),
                            labels=c("Female Medical Patients Seen (N;%)   ",
                                     "Female Clinic Enrollees   ","Campus Population")
                            ,breaks=c("spec_md_pt_ct_f","spec_clinic_en_f","spec_x_cam_popn"))+
