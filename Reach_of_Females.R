...
create_rc_fm_et <- function(myfile){
  ...
  data_set$x_cam_pct <-round((data_set$Med_Pt_Ct_F/data_set$Campus_Popn_F)*100)
  ...
  list_Sponsor_Level_Options <- c(select.list(option_text1,
    ...
    if(list_Sponsor_Level_Options == "Specific Sponsor"){
      ...
      new_data_set_1<-data.frame(spec_sitename,spec_cam_popn_f,spec_md_pt_ct_f,spec_x_cam_pop,spec_x_cam_pct)
      
      new_data_1<-melt(as.data.frame(new_data_set_1), id.vars=c('spec_sitename','spec_cam_popn_f'),
                measure.vars=c('spec_x_cam_pop','spec_md_pt_ct_f'))
      new_data_2<-melt(as.data.frame(new_data_set_1), id.vars=c('spec_sitename','spec_cam_popn_f'),
                measure.vars=c('spec_x_cam_pop','spec_md_pt_ct_f','spec_cam_popn_f'))
      
      new_data_3<-melt(as.data.frame(new_data_set_1), id.vars=c('spec_sitename','spec_cam_popn_f','spec_x_cam_pct'),
      ...
      if(list_identification=="Identified"){
        ggplot(data = new_data_1, aes(x=reorder(spec_sitename,value), value, fill=variable,label=value))+ geom_col(width=.5) + 
          ...
          theme_classic()+ annotate("text", x=new_sub_data$spec_sitename, y=new_sub_data$value, label= new_sub_data$dt_lbl,vjust=-.8,size=3)+
          
          geom_text(position=position_stack(vjust=.68),size=0,color="#d9d9d9",alpha=0)+
          scale_fill_manual(values=c("#d9d9d9","#604a7b"),
                            labels=c("Female Medical Patients Seen(N;%)   ","  Female Campus Population")
                            , breaks=c("spec_md_pt_ct_f","spec_x_cam_pop"))+
      ...
      new_data_set_1<-data.frame(sitename_all_sponsor,cam_popn_f_all_sponsor,md_pt_ct_f_all_sponsor,x_cam_popn_all_sponsor,hs_cam_popn_all_sponsor,x_cam_pct_all_sponsor)
      
      new_data_1<-melt(as.data.frame(new_data_set_1), id.vars=c('sitename_all_sponsor','cam_popn_f_all_sponsor','hs_cam_popn_all_sponsor'),
                measure.vars=c('x_cam_popn_all_sponsor','md_pt_ct_f_all_sponsor'))
      new_data_1<-na.omit(new_data_1)
      
      new_data_2<-melt(as.data.frame(new_data_set_1), id.vars=c('sitename_all_sponsor','cam_popn_f_all_sponsor','hs_cam_popn_all_sponsor'),
                measure.vars=c('x_cam_popn_all_sponsor','md_pt_ct_f_all_sponsor','cam_popn_f_all_sponsor'))
      new_data_2<-na.omit(new_data_2)
      
      
      new_data_3<-melt(as.data.frame(new_data_set_1), id.vars=c('sitename_all_sponsor','cam_popn_f_all_sponsor','hs_cam_popn_all_sponsor','x_cam_pct_all_sponsor'),
      ...
      new_data_set_2<-data.frame(spec_sitename,spec_cam_popn_f,spec_md_pt_ct_f,spec_x_cam_pop,spec_hs_cam_popn,spec_x_cam_pct)
      
      new_data_4<-melt(as.data.frame(new_data_set_2), id.vars=c('spec_sitename','spec_cam_popn_f','spec_hs_cam_popn'),
                measure.vars=c('spec_x_cam_pop','spec_md_pt_ct_f'))
      new_data_5<-melt(as.data.frame(new_data_set_2), id.vars=c('spec_sitename','spec_cam_popn_f','spec_hs_cam_popn'),
                measure.vars=c('spec_x_cam_pop','spec_md_pt_ct_f','spec_cam_popn_f'))
      
      new_data_6<-melt(as.data.frame(new_data_set_2), id.vars=c('spec_sitename','spec_cam_popn_f','spec_hs_cam_popn','spec_x_cam_pct'),
                measure.vars=c('spec_md_pt_ct_f'))
      ...
      if(list_identification == "Identified"){
        ggplot()+
          ...
          geom_text(position=position_stack(vjust=.68),size=0,color="#d9d9d9",alpha=0)+
          scale_fill_manual(values=c("#ccc1d4","#604a7b","#918e8e","#d9d9d9"),
